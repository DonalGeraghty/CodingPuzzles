...
public class GoodVsEvilTest {
    ...
    public void secondTest() {
        assertEquals("Battle Result: Good triumphs over Evil",
                battle("0 0 0 0 0 10", "0 1 1 1 1 0 0"));
    }
    ...
    private String battle(String goodAmounts, String evilAmounts) {

        int goodTotal = 0;
        int[] goodValues = new int[]{1, 2, 3, 3, 4, 10};
        String[] goodSplit = goodAmounts.split("\\s+");
        for (int i = 0; i < goodSplit.length; i++) {
            goodTotal += Integer.parseInt(goodSplit[i]) * goodValues[i];
        }

        int evilTotal = 0;
        int[] evilValues = new int[]{1, 2, 2, 2, 3, 5, 10};
        String[] evilSplit = evilAmounts.split("\\s+");
        for (int i = 0; i < evilSplit.length; i++) {
            evilTotal += Integer.parseInt(evilSplit[i]) * evilValues[i];
        }
